...
wss.on('connection',socket=>{let client:Client|undefined; socket.on('message',raw=>{try{const cmd=validCommand(JSON.parse(raw.toString()));const r=getRoom(cmd.roomCode);
 ...
 const action=actionFor({...cmd,playerId:client.playerId});if(!action)throw Error('Unknown command');r.state=reduce(r.state as CardfallState,action);r.version++;r.seen.set(cmd.commandId,{playerId:client.playerId,fingerprint});broadcast(r);
