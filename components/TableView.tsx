...
export function TableView() {
  ...
  const connect = async (nextRoom: string, displayName: string, gameId: GameId, gameSettings: Record<string,string|number|boolean>) => {
    ...
    socket.current = ws;
    ws.onopen = () => { setConnected(true); reconnectAttempt.current=0; ws.send(JSON.stringify({ type: 'JOIN_ROOM', roomCode: nextRoom, playerId: meRef.current, sessionToken: sessionToken.current || undefined, commandId: crypto.randomUUID(), name: displayName, gameId, settings: gameSettings })); };
    ...
    ws.onclose = () => { setConnected(false); if (screenRef.current==='room' && reconnectAttempt.current < 5) { const delay=Math.min(8000,500*Math.pow(2,reconnectAttempt.current++)); reconnectTimer.current=setTimeout(()=>connect(nextRoom,displayName,gameId,gameSettings),delay); } };
  };
  ...
  const enter = (nextCode: string) => { const cleanName = name.trim().slice(0, 24) || 'Guest'; const nextRoom = nextCode.trim().toUpperCase().replace(/[^A-Z0-9-]/g, '').slice(0, 12); if (!nextRoom) { setError('Enter a room code first.'); return; } const normalized=normalizeSettings(selectedGame,settings); setRoom(nextRoom); screenRef.current='room'; setScreen('room'); connect(nextRoom, cleanName, selectedGame, normalized); };
  ...
  if (screen === 'lobby') return <div className="shell lobby-shell"><header className="topbar"><div className="brand"><span className="mark">✦</span><span>Card Table <b>Arena</b></span></div><span className="safe-pill">GUEST PLAY · NO ACCOUNT · PLAY MONEY</span></header><main className="lobby lobby-wide"><div className="lobby-copy"><p className="eyebrow">CHOOSE YOUR TABLE</p><h1>Pick a game.<br/><em>Make it yours.</em></h1><p className="sub">Choose a classic, tune the table, then invite your friends.<br/>You can play as a guest—no account required.</p><div className="steps"><span className="active">01 Choose</span><span>02 Tune</span><span>03 Invite</span></div></div><div className="setup-panel"><div className="game-grid">{GAME_CATALOG.map(game=><button key={game.id} className={`game-tile ${selectedGame===game.id?'chosen':''} accent-${game.accent}`} onClick={()=>{setSelectedGame(game.id);setSettings(defaultSettings(game.id));}}><span className="game-symbol">{game.id==='cardfall'?'✦':game.id==='blackjack'?'♠':game.id==='holdem'?'♣':game.id==='solitaire'?'◈':'◆'}</span><strong>{game.name}</strong><small>{game.tagline}</small><em>{game.players}</em></button>)}</div><div className="selected-summary"><div><span className="panel-kicker">SELECTED GAME</span><h2>{selectedDefinition.name}<small>{selectedDefinition.tagline}</small></h2></div><span className="mode-pill">{selectedDefinition.players}</span></div><div className="rules-settings"><div className="rules"><span className="panel-kicker">HOW IT WORKS</span>{selectedDefinition.rules.map((rule,i)=><p key={rule}><b>{i+1}</b>{rule}</p>)}</div><div className="settings"><span className="panel-kicker">TABLE SETTINGS</span>{selectedDefinition.fields.map(field=><label className="setting-field" key={field.key}>{field.label}{field.type==='number'?<input type="number" min={field.min} max={field.max} value={Number(settings[field.key])} onChange={e=>setSettings({...settings,[field.key]:Number(e.target.value)})}/>:<select value={String(settings[field.key])} onChange={e=>setSettings({...settings,[field.key]:e.target.value})}>{field.options?.map(o=><option key={String(o.value)} value={String(o.value)}>{o.label}</option>)}</select>}<small>{field.help}</small></label>)}</div></div><label className="field-label">Your table name<input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Alex" maxLength={24}/></label><div className="lobby-actions"><button className="primary" onClick={()=>enter(roomCode())}>Create {selectedDefinition.name} table <span>→</span></button><div className="join-line"><input value={code} onChange={e=>setCode(e.target.value.toUpperCase())} placeholder="HAVE A ROOM CODE?" maxLength={12}/><button className="secondary" onClick={()=>enter(code)}>Join table</button></div></div>{error&&<p className="form-error">{error}</p>}<p className="guest-note">✦ You are joining as a guest. Your name is only used at this table.</p></div></main><footer><span>PRIVATE ROOMS · NO TRACKING</span><span>{GAME_CATALOG.length} TABLE MODES <i/> CARD TABLE ARENA</span></footer></div>;
